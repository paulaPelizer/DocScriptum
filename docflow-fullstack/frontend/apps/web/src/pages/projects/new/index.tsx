...
type UserMap = Record<number, { id: number; name: string; email: string }>;
function buildUserMap(groups: Record<string, User[]>): UserMap {
  const map: UserMap = {};
  Object.values(groups).forEach(arr => arr.forEach(u => (map[u.id] = u)));
  return map;
}
...
export default function NewProjectPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // 1) status + datas (backend exige dd/MM/yyyy)
      const statusInicial = projectData.status || null;
      const dataInicio = toDDMMYYYY(projectData.startDate);
      const dataPrevistaConclusao = toDDMMYYYY(projectData.endDate);

      // 2) disciplinas -> DTO exigido
      const userMap = buildUserMap(userGroups);
      const toList = (ids?: number[]) =>
        (ids || [])
          .map(id => userMap[id])
          .filter(Boolean)
      ...
      const disciplinasDTO = (disciplinas || []).map(d => {
        ...
        const tipos = Array.from(countByName.entries()).map(([tipo, quantidade]) => ({ tipo, quantidade }));

        return {
          disciplinaId: d.id,
          disciplinaNome: d.name || "",
          destinatarioCliente: toList(d.destinatarios?.cliente) || null,
          destinatarioInterno: toList(d.destinatarios?.interno) || null,
          tipos,
        };
      });

      // 3) marcos -> DTO exigido
      const marcosDTO = (milestones || [])
        .filter(m => (m.name || "").trim())
        .map(m => ({
          marcoContratual: (m.name || "").trim(),
          dataLimite: toDDMMYYYY(m.dueDate),
          descricao: m.description || null,
        }));

      // 4) payload final conforme CreateProjectDTO
      const payload = {
  ...
  const addDocument = (disciplinaId: number) => {
    setDisciplinas((prev) =>
      prev.map((d) =>
        d.id === disciplinaId
          ? { ...d, documents: [...d.documents, { id: Date.now(), name: "", dueDate: "" } as ProjectDoc] }
          : d,
      ),
    );
  };
  ...
  const updateDocument = (disciplinaId: number, docId: number, field: keyof ProjectDoc, value: string) => {
    setDisciplinas((prev) =>
      prev.map((d) =>
        d.id === disciplinaId
