...
export default function EditMultipleProjectsPage() {
  ...
  useEffect(() => {
    ...
    const mockProjects = [
      { id: 1, name: "Projeto Alpha", client: "Empresa ABC", status: "Em andamento" },
      { id: 2, name: "Expansão Sede", client: "Construtora XYZ", status: "Em andamento" },
      { id: 3, name: "Reforma Unidade 3", client: "Indústria 123", status: "Em revisão" },
    ];

    setSelectedProjects(mockProjects.filter((p) => ids.includes(p.id)));
  }, [searchParams]);
      ...
      <main className="flex-1 p-4 md:p-6">
        <div className="container mx-auto max-w-4xl">
          <PageHeader
            ...
            description={`Editar ${selectedProjects.length} projeto${
              selectedProjects.length === 1 ? "" : "s"
            } selecionado${selectedProjects.length === 1 ? "" : "s"}`}
          >
            <Link to="/projects">
